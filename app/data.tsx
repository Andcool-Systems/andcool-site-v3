const fu = <>Самый простой способ обмениваться файлами со своими друзьями. Проект позволяет обмениваться файлами через ссылки, а получателю не обязательно иметь аккаунт на сайте.</>
const fu_more = <>
    ...
    Все смежные проекты на данный момент используют официальный <a href="https://github.com/Andcool-Systems/FileUploader-python">пакет</a> для Python, в котором описан весь <a href="https://github.com/Andcool-Systems/File-uploader/blob/main/README.md">API</a>.<br /><br />
...
const ppl_more = <>
    ...
    <b>2. <a href="https://t.me/pplBandageBot" target="_blank">Telegram бот v2.0</a></b> (06.03.2024 – 02.06.2024) В связи со сложностью обновления старого бота был создан новый, где использовался Aiogram 3. Версия 2.0 имела обновлённый клавиатурный интерфейс и оптимизированный метод рендера.<br />
...
const weather = <>Виджет погоды в виде изображения, который Вы можете установить куда угодно, например, в README своего профиля GitHub.</>
const weather_more = <>
    ...
    Рабочий пример:<br />
    <img style={{ maxWidth: "100%" }} src="https://weather.andcool.ru/api?place=moscow&timezone=gmt3&language=ru" alt="weather widget" />
</>


const bf = <>Интерпретатор языка BrainFuckA, написанный на языке Python. BrainFuckA - это brainfuck-подобный язык программирования с дополнениями для удобства.</>
const bf_more = <>
    Проект разрабатывался для практики в программировании, а так же из-за желания понять, как работает интерпретация языка изнутри.<br /><br />
    Впоследствии язык был доработан добавлением 8 дополнительных инструкций, что увело его от оригинального концепта, хотя код на чистом BrainFuck так же стабильно работает на этом интерпретаторе.
...
const oauth = <>Сервис, позволяющий создавать аутентификацию через Minecraft аккаунт без надобности запрашивать логин и пароль у пользователя.</>
const oauth_more = <>
    Проект разрабатывался как замена сервису mc-oauth.com, так как на момент создания он был отключён.<br /><br />
    Вначале был создан серверный плагин на версию 1.20. При входе на сервер плагин генерировал 6-значный код, с которым ассоциировал никнейм и uuid зашедшего игрока.
    Однако сервер Minecraft тратил слишком много ресурсов сервера впустую. В связи с этим было принято
    решение разработать сервер с нуля, имеющий только поток авторизации для клиентов. Разработка нового сервера авторизации значительно снизила нагрузку на хост и позволила использовать его в реальных целях.
</>


const newAndcool = <>Новый персональный сайт, который должен был заменить текущий. Однако разработка была приостановлена из-за отсутствия времени и малой содержательности сайта.</>


const activity = <>Сервис, позволяющий регистрировать активность в редакторе кода и получать его через RestFul API.</>;
const activity_more = <>
    Данный проект разрабатывался как часть для нового личного сайта.
    Однако из-за остановки разработки родительского проекта, API был переделан под виджет, который можно добавить в свой GitHub README.<br />
    Основной идеей проекта является установка специального расширения в свой редактор кода (на данный момент только VSCode) и получение виджета по ссылке.<br /><br />
    Рабочий пример: <br />
